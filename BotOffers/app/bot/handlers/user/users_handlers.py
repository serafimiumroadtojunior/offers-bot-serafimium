...
from app.bot.keyboards import (
    ...
    customed_keyboard, PayingCallback
)
...
@user_router.message(
    ...
    PayingCallback.filter(F.action == "support_donate")
)
async def on_successful_payment(
    message: Message,
    redis: Redis,
    callback: PayingCallback,
    i18n: I18nContext,
    settings: Settings,
    bot: Bot
) -> Optional[Message]:
...
async def refund(
    ...
    charge_id, stars_amount = await get_user_check(
        user_id=user_id,
        i18n=i18n,
        locale=locale
    )
